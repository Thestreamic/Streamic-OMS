...
def is_broadcast_item(title: str, description: str = "") -> bool:
    ...
    if any(k in text for k in _REJECT_KEYWORDS):
        return False
    if any(k in text for k in LOW_SIGNAL_TITLE_PATTERNS):
        return False
...
MAX_ITEMS_PER_FEED = 6    # Stricter quality pool: fewer, better candidates per source
...
DIRECT_FEEDS = [
    # High-signal trade and vendor sources only
    ...
    'https://www.thebroadcastbridge.com/feed/',
...
TRUSTED_SOURCES = {
    ...
    'Haivision', 'Telestream Blog', 'Harmonic', 'AWS Media Blog', 'Vizrt', 'Ross Video', 'Avid'
}

LOW_SIGNAL_TITLE_PATTERNS = [
    'music video', 'trailer', 'oscar', 'student', 'education', 'behind the scenes',
    'creature effects', 'bosnian knight', 'guide to ai video', 'beginner', 'budget studio',
    'namibia runs streaming and av events', 'credit-based model', 'political campaign video'
]
