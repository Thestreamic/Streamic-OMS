...
DO NOT summarize the article. Instead, expand it into a high-value, original analysis piece that provides genuine expert depth.
...
─────────────────────────────────────────────────────────────
1. Total length: 1200–1500 words. Do NOT write less than 1200 words.
2. Use all seven section headings exactly as written above.
...
function buildAdSlot() {
  ...
  <ins class="adsbygoogle" style="display:block" data-ad-client="${ADS_ID}"
...
function buildPostsPage(arts) {
  const cards = arts.map((art, i) => {
    ...
    const catLabel = (art.category||'featured').replace(/-/g,' ').replace(/\b\w/g,c=>c.toUpperCase());
    return `<article class="nc${i===0?' nc:first-child':''}" style="${i===0?'grid-column:span 2;flex-direction:row':''}">
  ${art.image ? `<div class="nc-img" style="${i===0?'width:52%;flex-shrink:0;aspect-ratio:auto':'aspect-ratio:16/9'}">
    <a href="posts/${art.slug}.html" tabindex="-1" aria-hidden="true">
      <img src="${esc(art.image)}" alt="${esc(art.title)}" loading="${i===0?'eager':'lazy'}">
    </a>
  </div>` : ''}
  <div class="nc-body">
    <div class="nc-cat" style="color:${catColor}">${esc(catLabel)}</div>
    <h2 class="nc-hl${i===0?' nc-hl-featured':''}">
      <a href="posts/${art.slug}.html">${esc(art.title)}</a>
    </h2>
    <p class="nc-sum">${esc(art.dek)}</p>
    <div class="nc-foot">
      <span class="nc-src">${esc(art.sourceName)}</span>
      <a href="posts/${art.slug}.html" class="nc-read">${art.wordCount > 800 ? 'Read Full Analysis →' : 'Read Article →'}</a>
    </div>
  </div>
</article>`;
  }).join('\n');
...
<head>
  ...
  <meta name="description" content="In-depth AI-assisted broadcast technology analysis. Original articles on streaming, cloud production, IP infrastructure, newsroom tech, and post-production.">
  <meta name="robots" content="index,follow">
...
<main>
  <div class="w">
    <div class="cat-hdr">
      <h1>All Articles</h1>
      <p>Original AI-assisted analysis of broadcast and streaming technology. Every article is 1,000+ words of original editorial commentary.</p>
    </div>
    ${buildAdSlot()}
    <ul class="news-grid" style="margin-top:32px">${cards}</ul>
    ${buildAdSlot()}
  </div>
</main>
${buildFooter()}
...
async function main() {
  ...
  for (let i = 0; i < newItems.length; i++) {
    ...
    const bodyHtml = generatedText
      ? articleTextToHtml(generatedText, item.link, item.sourceName)
