...
def intelligence_feed_section(arts, base=""):
    """3-column Apple Newsroom-style card grid for the homepage intelligence feed."""
    # Pull latest 15 RSS articles (non-editorial)
...
<article style="background:#fff;border-radius:12px;border:1px solid #eee;overflow:hidden;display:flex;flex-direction:column;transition:box-shadow 0.2s ease,transform 0.2s ease">
  ...
  <div style="padding:16px 18px 18px;display:flex;flex-direction:column;flex:1">
    ...
    <h3 style="font-family:var(--serif);font-size:17px;line-height:1.35;letter-spacing:-.02em;color:var(--ink);margin:0 0 10px;flex:1">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-top:auto;padding-top:12px;border-top:1px solid #f0f0f0">
      <span style="font-size:11px;color:var(--ink4)">{src_dom} · <time>{dt}</time></span>
      <a href="{href}" style="font-size:12px;font-weight:600;color:var(--blue);text-decoration:none;white-space:nowrap">Read →</a>
    </div>
  </div>
</article>"""
  ...
  <div style="display:flex;align-items:baseline;justify-content:space-between;margin-bottom:8px;padding-bottom:12px;border-bottom:2px solid var(--ink)">
    ...
    <a href="posts.html" style="font-size:13px;font-weight:600;color:var(--blue);text-decoration:none;white-space:nowrap;flex-shrink:0;margin-left:16px">View all articles →</a>
  </div>
  <p class="section-desc" style="font-size:14px;color:var(--ink3);margin:0 0 24px;line-height:1.6">Deep-dive reporting on the intersection of cloud production, AI-driven media workflows, and global streaming infrastructure.</p>
  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:20px">
    <style>
      @media(max-width:768px){{.intel-grid{{grid-template-columns:1fr!important}}}}
      @media(max-width:1024px) and (min-width:769px){{.intel-grid{{grid-template-columns:repeat(2,1fr)!important}}}}
    ...
    <div class="intel-grid" style="display:grid;grid-template-columns:repeat(3,1fr);gap:20px;grid-column:1/-1">
      {cards_html}
      {disclosure}
    </div>
  </div>
