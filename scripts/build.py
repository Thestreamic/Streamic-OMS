...
def news_grid(arts, base=""):
    """Render a list of articles as ic-card grid (replaces old bento grid)."""
    if not arts:
        return ""
    cards = "".join(news_card(a, base=base, is_first=False) for a in arts)
    return f"""<ul class="ic-grid-list">
{cards}
</ul>"""


def news_card(a, base="", is_first=False):
    """Square Apple Newsroom-style card — consistent with intelligence feed.
    No AI summaries shown. Image on top for featured first card, no image for standard.
    """
    cat     = a.get("category","featured")
    cinfo   = CAT.get(cat, CAT["featured"])
    slug_   = a.get("slug","")
    href    = f"{base}articles/{slug_}.html"
    img     = e(a.get("image_url","") or a.get("img",""))
    fb      = f"{base}assets/fallback.jpg"
    title   = e(a.get("title",""))
    src_dom = e((a.get("source_domain","") or "").replace("https://","").replace("www.","").split("/")[0].upper())
    ...
    if is_first and img:
        # First card: image on top (featured style)
        return f"""<li class="bento-item bento-first">
<article class="ic-card ic-card-featured">
  ...
  <div class="ic-inner">
    <div class="ic-top">
      <span class="ic-cat" style="color:{cat_col}"><span class="ic-dot" style="background:{cat_col}"></span>{cat_lbl}</span>
    </div>
    <h3 class="ic-title ic-title-lg"><a href="{href}" style="color:inherit;text-decoration:none">{title}</a></h3>
    <div class="ic-meta"><span class="ic-src">{src_dom}</span><time class="ic-date">{dt}</time></div>
  </div>
  <div class="ic-footer"><a href="{btn_href}"{btn_tgt} class="ic-btn">{btn_txt} &rarr;</a></div>
</article></li>"""

    return f"""<li class="bento-item">
<article class="ic-card">
  <div class="ic-inner">
    <div class="ic-top">
      <span class="ic-cat" style="color:{cat_col}"><span class="ic-dot" style="background:{cat_col}"></span>{cat_lbl}</span>
    </div>
    <h3 class="ic-title"><a href="{href}" style="color:inherit;text-decoration:none">{title}</a></h3>
    <div class="ic-meta"><span class="ic-src">{src_dom}</span><time class="ic-date">{dt}</time></div>
  </div>
  <div class="ic-footer"><a href="{btn_href}"{btn_tgt} class="ic-btn">{btn_txt} &rarr;</a></div>
</article></li>"""
...
def hero_block(a, base=""):
    ...
    fb    = f"{base}assets/fallback.jpg"
    title = e(a.get("title",""))
    dek   = e((a.get("card_summary") or a.get("dek") or "")[:180])
    wc    = a.get("word_count", 800)
    dt    = d(a.get("published",""))
    src_name = e((a.get("source_domain","") or "").replace("www.","").split(".")[0].upper())

    # Broadcast topic pills for bottom strip
    topics = [
        ("Cloud Playout", f"{base}cloud.html"),
        ("IP Infrastructure", f"{base}infrastructure.html"),
        ("MAM & Workflow", f"{base}ai-post-production.html"),
        ("Streaming Tech", f"{base}streaming.html"),
        ("Newsroom Systems", f"{base}newsroom.html"),
        ("Graphics & VFX", f"{base}graphics.html"),
    ]
    pills = "".join(
        f'<a href="{ph}" class="hero-topic-pill">{pt}</a>'
        for pt, ph in topics
    )

    return f"""<section class="hero">
  <div class="hero-ticker">
    <span class="hero-ticker-badge">Live</span>
    <span class="hero-ticker-text">
      <strong>Broadcast &amp; Streaming Technology</strong> &#8212;
      ST 2110 &middot; Cloud Playout &middot; MAM &middot; IP Infrastructure &middot; AI &amp; Automation &middot; NAB 2026
    </span>
  </div>
  <div class="hero-inner">
    ...
    <div class="hero-body">
      <span class="hero-tag" style="background:{cinfo['color']}">{cinfo['label']}</span>
      ...
      <a href="{href}" class="hero-cta">Read Analysis &rarr;</a>
    </div>
  </div>
  <div class="hero-topics">
    <span class="hero-topics-label">Trending:</span>
    {pills}
  </div>
</section>"""
...
def diversity_shuffle(arts, limit=15):
    """Round-robin across categories AND enforce max 1 article per source per round.
    Sorts each bucket by date (newest first) before round-robin selection.
    Priority: cloud, infrastructure, ai-post-production, newsroom,
              streaming, playout, featured, graphics, items.
    """
    priority = ["cloud","infrastructure","ai-post-production","newsroom",
                "streaming","playout","featured","graphics","items","featured_priority"]

    # Build buckets sorted newest-first, max 1 per source vendor
    buckets = {}
    for a in arts:
    ...
    # Sort each bucket by published date newest-first
    for cat in buckets:
        buckets[cat].sort(key=lambda x: x.get("published",""), reverse=True)

    # Deduplicate: max 1 article per source domain per bucket slot
    deduped = {}
    for cat, items in buckets.items():
        seen_src = {}
        deduped[cat] = []
        for a in items:
            src_key = (a.get("source_domain","") or "").replace("www.","").split(".")[0].lower()[:20]
            if src_key not in seen_src:
                deduped[cat].append(a)
...
def intelligence_feed_section(arts, base=""):
    ...
    """
    rss = diversity_shuffle(arts, limit=15)
    if not rss:
        return ""
...
def featured_page(arts):
    ...
    rss_arts  = [a for a in diversity_shuffle(arts, limit=15)
                 if not hero_art or a["slug"] != hero_art["slug"]][:12]
