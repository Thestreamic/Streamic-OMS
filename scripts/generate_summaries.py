...
# ── Prompt builders ───────────────────────────────────────────────────────────
_CARD_PROMPT = """You are a senior broadcast technology analyst at The Streamic.
Write a factual 2-paragraph summary for broadcast engineers.

DOMAIN: {domain_label}
FOCUS: {domain_focus}
USE TERMS FROM: {domain_terms}
GUARDRAIL: {domain_guardrail}

PARAGRAPH 1 (60-80 words): State exactly what happened — company, product, specific action, location if known.
PARAGRAPH 2 (60-80 words): Specific technical or operational implication using domain-appropriate terminology.

FORBIDDEN: "this highlights", "this underscores", "this reflects", "organizations should consider",
"in today's landscape", "rapidly evolving", "plays a key role", "gap between what is technically possible",
"vendor roadmap signals", "evaluation timelines".

Output plain text only (no HTML). 120-160 words maximum.
...
_ARTICLE_PROMPT = """You are a senior broadcast technology analyst writing for The Streamic at TV Tech / IABM / SVG Europe level.
Your readers are broadcast CTOs, infrastructure engineers, and media architects.

DOMAIN: {domain_label}
FOCUS: {domain_focus}
TERMINOLOGY: Use terms from — {domain_terms}
GUARDRAIL: {domain_guardrail}

Write a 700-word technical analysis using EXACTLY this HTML structure:

<h2>Technical Analysis: [Write a specific fact-based headline — NOT a restatement of the title]</h2>

<p>[Introduction — 2-3 sentences. State what happened, who announced it, and what technology is involved. No filler.]</p>

<h3>Operational ROI</h3>
<p>[How does this technology save money or time for a broadcast CTO? Quantify where possible. Reference: {domain_roi}. Name the specific workflow step this affects.]</p>
<p>[Second paragraph: compare to the existing approach or incumbent technology. What changes operationally?]</p>

<h3>Security &amp; Integration</h3>
<p>[How does this fit into an existing broadcast IT stack? What are the cybersecurity implications? Reference relevant standards or compliance requirements using domain terms: {domain_terms}.]</p>

<h3>Engineering Takeaways</h3>
<ul>
<li><strong>Deployment:</strong> [Specific deployment requirement — hardware spec, network bandwidth, software dependency, or certification needed]</li>
<li><strong>Compatibility:</strong> [Named standard or protocol compliance — be specific, e.g. SMPTE ST 2110-20, NMOS IS-04, or S3-compatible API]</li>
<li><strong>What to Watch:</strong> [One forward-looking item: a standards update, vendor roadmap point, or market development engineers should track]</li>
</ul>

<p>[Closing — 2 sentences maximum. What does this signal for the market? What should engineering teams do next? No generic phrases.]</p>

STRICT RULES:
- Temperature: 0.3 — factual only, zero invention
- 650-750 words of body content
- FORBIDDEN words: "this highlights", "this underscores", "this reflects", "in today's landscape",
  "important to note", "rapidly evolving", "plays a key role", "gap between",
  "vendor roadmap signals continued", "evaluation timelines"
- {domain_guardrail}
- Output valid HTML only: <h2>, <h3>, <p>, <ul>, <li>, <strong>. No markdown.
...
Write the complete structured technical analysis now:"""
...
def main():
    ...
    for item in items_to_process:
        ...
        try:
            ...
            # Classify domain before calling Groq
